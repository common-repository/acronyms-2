...
/*
  ...
  Description: This is a plugin to wrap acronyms in posts and comments with appropriate <samp>&lt;acronym&gt;</samp> or <samp>&lt;abbr&gt;</samp> markup. Allows users to manage lists of acronyms and abbreviations through admin interface. Based on Joel Pan's Acronyms plugin, updated for WordPress 3.0 and HTML5.
  Version: 2.0.10
  Author: Chris Cox
...
class Acronyms {
  ...
  static function get_textdomain() {
	return "Acronyms";
   ...
   *
   */
  static function management_handler() {
	$domain = Acronyms::get_textdomain();
   ...
   * @return void
   * */
  static function manage_acronyms() {
	$domain = Acronyms::get_textdomain();
	...
	<div class="wrap">
	  ...
	  <form class="search-form" method="get" action="">
		<p class="search-box" id="post-search">
		  ...
		  <input type="text" id="post-search-input" name="search" value="<?php echo isset( $_GET[ 's' ] ) ? attribute_escape( stripslashes( $_GET[ 's' ] ) ) : ''; ?>" />
	  ...
	  <div id="col-container">
		...
		<div id="col-left">
		  <div class="col-wrap">
			<?php
			if ( isset( $_GET[ 'action' ] ) && 'edit' == $_GET[ 'action' ] && !empty( $_GET[ 'acronym' ] ) ) {
  ...
  /**
   * Check if the active theme is served as HTML5
   */
  static function theme_html5_check() {
  ...
  static function acronym_replace( $text ) {
	$acronyms	 = get_option( 'acronym_acronyms' );
  ...
  static function show_acronym_list( $s, $n, $p = '1' ) {
	$domain = Acronyms::get_textdomain();
  ...
  static function show_pagination_bar( $s, $n, $p, $t, $filter = false ) {
  ...
  static function add_acronym_form( $acronym = '', $fulltext = '' ) {
	$domain = Acronyms::get_textdomain();
	...
	<?php echo $form ?>
	  ...
	  <div class="form-field form-required">
		...
		<input name="acronym" id="acronym" type="text" value="<?php echo attribute_escape( $acronym ); ?>" size="20" <?php
		   if ( 'edit-acronym' == $action )
	...
	if ( 'edit-acronym' == $action )
	  ...
	  ?>
		<input type="submit" class="button button-primary" name="submit" value="<?php echo $submit_text ?>" />
	  </p>
	</form>
	</div>
	<?php
  ...
  static function update( $acronym, $fulltext ) {
  ...
  static function delete( $acronym ) {
	$acronyms = get_option( 'acronym_acronyms' );
  ...
  static function count_acronyms( $s = '' ) {
	$acronyms = get_option( 'acronym_acronyms' );
	if ( empty( $s ) )
